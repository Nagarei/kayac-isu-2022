...
func getRecentPlaylistSummaries(ctx context.Context, db connOrTx, userAccount string) ([]Playlist, error) {
	...
	if err := db.SelectContext(
		ctx,
		&playlists,
		" SELECT pl.id as id, pl.ulid as ulid, pl.name as name, pl.is_public as is_public, pl.created_at as created_at, pl.updated_at as updated_at, display_name, account, COALESCE(count,0) as favorite_count"+
			" FROM playlist as pl INNER JOIN user ON user.`account` = pl.`user_account`"+
			" LEFT JOIN favorite_count ON favorite_count.playlist_id = pl.id"+
